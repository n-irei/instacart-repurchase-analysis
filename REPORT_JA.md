...
準備処理は52.9秒、peak working set約3.53GBでした。学習・再学習・CSV生成は108.7秒、peak working set約3.90GBでした。実測は`runtime_train.json`に記録しています。今回の機器にはメモリ余裕があり、主集計は全priorを読み込みました。独立再検証は100万行チャンクです。主パイプライン全体が任意の低メモリ機器で動くストリーミング実装であるとは主張しません。低メモリ環境ではuser_idによるパーティション集計等が追加で必要です。
